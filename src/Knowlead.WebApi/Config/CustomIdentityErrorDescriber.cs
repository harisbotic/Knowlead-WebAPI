    ...
    public class CustomIdentityErrorDescriber : IdentityErrorDescriber
        {
            ...
            public override IdentityError PasswordMismatch() { return new IdentityError { Code = myPassword, Description = "Incorrect password." }; }
            ...
            public override IdentityError InvalidEmail(string email) { return new IdentityError { Code = myEmail, Description = $"Email '{email}' is invalid."  }; }
            public override IdentityError DuplicateUserName(string userName) { return new IdentityError { Code = null, Description = null }; }
            public override IdentityError DuplicateEmail(string email) { return new IdentityError { Code = myEmail, Description = $"Email '{email}' is already taken."  }; }
            ...
            public override IdentityError PasswordTooShort(int length) { return new IdentityError { Code = myPassword, Description = $"Passwords must be at least {length} characters."  }; }
            public override IdentityError PasswordRequiresNonAlphanumeric() { return new IdentityError { Code = myPassword, Description = "Passwords must have at least one non alphanumeric character." }; }
            public override IdentityError PasswordRequiresDigit() { return new IdentityError { Code = myPassword, Description = "Passwords must have at least one digit ('0'-'9')." }; }
            public override IdentityError PasswordRequiresLower() { return new IdentityError { Code = myPassword, Description = "Passwords must have at least one lowercase ('a'-'z')." }; }
            public override IdentityError PasswordRequiresUpper() { return new IdentityError { Code = myPassword, Description = "Passwords must have at least one uppercase ('A'-'Z')." }; }
        }
}